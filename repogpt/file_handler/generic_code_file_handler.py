...
class GenericCodeFileHandler(AbstractHandler):
    ...
    def extract_vscode_ext_codelens(
        ...
        tree = self.parser.parse(bytes(file_data, "utf8"))
...
class CSharpFileHandler(GenericCodeFileHandler):
    def __init__(self):
        super().__init__(
            lang="c_sharp",
            function_name_node_type="method_declaration",
            class_name_node_type="class_declaration",
            function_node_type="method_declaration",
            class_node_type="class_declaration",
            method_node_type="method_declaration",
            class_internal_node_type="declaration_list",  # You may need to adjust this based on C# grammar specifics
            parent_class_node_type="base_list",
            parent_class_name_node_type="identifier",
            method_name_node_type="identifier",
            function_output_node_type="type",
            function_parameters_node_type="parameter_list",
            root_node_type="source_file",
            function_query="(method_declaration name: (identifier) @name) @definition.method",
            class_query="(class_declaration name: (identifier) @name) @definition.class",
            method_query="(method_declaration name: (identifier) @name) @definition.method",
        )


class TerraformFileHandler(GenericCodeFileHandler):
    def __init__(self):
        super().__init__(
            lang="hcl",
            function_name_node_type="identifier",  # HCL does not use functions in the traditional sense
            class_name_node_type="identifier",  # Using 'identifier' for module names
            function_node_type="module_call",  # Treating modules as 'functions'
            class_node_type="resource",  # Using resources as 'classes'
            method_node_type="block",  # Blocks can be considered as 'methods' within resources
            class_internal_node_type="block",  # Blocks are a key concept in HCL
            parent_class_node_type="block",  # Parent class concept does not directly apply in HCL
            parent_class_name_node_type="identifier",  # Not directly applicable in HCL
            method_name_node_type="identifier",  # Block names within resources
            function_output_node_type="output",  # Output values in HCL
            function_parameters_node_type="input",  # Input variables for modules
            root_node_type="hcl_file",  # Root node type for HCL files
        )
        # Define custom queries for HCL's syntax
        self.function_query = """
            (module_call) @module
        """
        self.class_query = """
