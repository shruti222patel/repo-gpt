...
from repogpt.agents.autogen.llm_agent import LLMAgent
from repogpt.agents.autogen.user_proxy_function_call_agent import (
    UserProxyFunctionCallAgent,
)
from repogpt.agents.repo_comprehender import (
    get_relative_path_current_directory_structure,
    get_relative_path_directory_structure,
    get_relative_path_directory_structure_string,
)
from repogpt.openai_service import OpenAIService, num_tokens_from_string
...
class RepoQnA:
    def __init__(
        self, queue, loop, question, root_path, embedding_path=None, openai_api_key=None
    ):
        self.queue = queue
        self.loop = loop
        ...
        self.embedding_path = (
            ...
            else self.root_path / ".repogpt/code_embeddings.pkl"
        )
    ...
    def create_function_augmented_config(self, config):
        THRESHOLD_DIR_TOKENS = 500
        print(f"{get_relative_path_directory_structure_string(self.root_path)=}")
        if (
            num_tokens_from_string(
                get_relative_path_directory_structure_string(self.root_path)
            )
            < THRESHOLD_DIR_TOKENS
        ):
            file_search_functions = [
                {
            ...
            {get_relative_path_directory_structure(self.root_path)}
            """,
                            }
                        },
                        "required": ["file_path"],
            ...
            file_search_functions = [
                {
                    "name": "ls",
                    "description": "Use this function to list the files and directories in the user's codebase.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "directory": {
                                "type": "string",
                                "description": f"""The path of the directory you want to list the files and directories of. Here are the root code folder's files and directories:
{current_dir_files}""",
                                "default": "./",
                            }
                        },
                ...
                },
                {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            "file_path": {
                                "type": "string",
                                "description": f"""The file path of the file you want to retrieve the raw contents of.""",
                            }
                        },
                        "required": ["file_path"],
                    },
        ...
        common_function_list.extend(file_search_functions)
        aug_config = {
            "functions": common_function_list,
            "config_list": config,
            "request_timeout": 120,
        }
...
```""".strip(),
        ...
        asyncio.run_coroutine_threadsafe(
            self.queue.put(
                {
                    "message": self.question_answer
                    or "Review the last answer from the CodeRepo Librarian. I'm having difficulty understanding it.",
                    "sender": FINISHED,
                }
            ),
            self.loop,
    ...
    class RepoGptAgent(UserProxyFunctionCallAgent):
        def __init__(
            self,
            queue,
            loop,
        ...
        def ls(self, directory=None):
            ...
            return "\n".join(
                get_relative_path_current_directory_structure(directory, self.root_path)
            )
        ...
        def semantic_search(self, query):
            result = self.search_service.semantic_search_similar_code(query)
            # logger.info(f"Searching the codebase for: {query}")
            return convert_search_df_to_json(result)
        ...
        def view_raw_file_contents(self, file_path):
            ...
            tokens = tiktoken.get_encoding("cl100k_base").encode(contents)
            threshold = 2048
...
def test():
    ...
    # REPO_QUESTION = "What is the testing framework used in this repo?"
    # REPO_QUESTION = "What is the testing framework used in this repo?"
    root_path = "/Users/shrutipatel/projects/work/repo-gpt"
    embedding_path = (
        "/Users/shrutipatel/projects/work/repo-gpt/.repogpt/code_embeddings.pkl"
    )
    repo_qna = RepoQnA(REPO_QUESTION, root_path, embedding_path)
