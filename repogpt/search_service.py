...
class SearchService(metaclass=Singleton):
    ...
    def semantic_search_similar_code(self, query: str, matches_to_return: int = 3):
        embedding = self.openai_service.get_embedding_sync(query)
        # logger.verbose_info("Searching for similar code...")

        # Calculate similarities and store them in a new column in the DataFrame
        self.df["similarity"] = self.df["code_embedding"].apply(
            lambda x: x.dot(embedding)
        )

        # Sort the DataFrame by the similarity column in descending order and get the top matches
        top_matches = self.df.sort_values(by="similarity", ascending=False).head(
            matches_to_return
        )
    ...
    async def question_answer(self, question: str) -> AsyncGenerator[str, None]:
        ...
        return self.openai_service.query_stream(question, code)
    ...
    async def explain(self, code: str, output_html: bool = False):
        format = "html" if output_html else "markdown"

        return self.openai_service.query_stream(
